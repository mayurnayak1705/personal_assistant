...
from app.memory.session_store import add_tool_events, add_turn, get_tool_events, get_turns, pop_session
from app.core.tokens import count_tokens
from app.persistence.postgres_insert import insert_chat_history_batch
from app.persistence.postgres_search import fetch_conversation_history, fetch_user_facts, fetch_user_profile_name
...
from app.memory.action_history_store import fetch_action_history, save_action_history_events
from app.memory.follow_up_suggestions import choose_follow_up
from app.memory.working_context import build_tool_event
from app.memory.working_context_store import fetch_working_context, save_working_context_events
from app.features.briefing.service import generate_daily_briefing
from app.features.briefing.store import get_daily_briefing_preference
from app.features.google.oauth import (
    begin_authorization,
...
from app.features.expenses.email_ingestion import pending_imports, resolve_import, scan_transaction_emails
from app.core.debug import debug
from app.features.profile.store import DEFAULT_USER_ID, get_user_profile, save_user_profile
from app.features.wellness.store import get_profile as get_wellness_profile, save_profile as save_wellness_profile, add_log as add_wellness_log, report as wellness_report, due_prompts as wellness_due_prompts
from app.features.finance.store import watchlist_report as finance_watchlist_report, notifications as finance_due_notifications
...
@router.get("/wellness/profile")
async def wellness_profile(user_id: str = DEFAULT_USER_ID):
    try:
        profile = await asyncio.to_thread(get_wellness_profile, user_id)
        return {"profile": profile, "needs_setup": profile is None}
    except Exception as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc

@router.post("/wellness/profile")
async def update_wellness_profile(request: WellnessProfileRequest):
    try:
        return {"profile": await asyncio.to_thread(save_wellness_profile, request.user_id, request.data), "needs_setup": False}
    except Exception as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc

@router.post("/wellness/logs")
async def create_wellness_log(request: WellnessLogRequest):
    try:
        log_date = datetime.fromisoformat(request.log_date).date() if request.log_date else None
        return {"log": await asyncio.to_thread(add_wellness_log, request.user_id, request.kind, request.data, request.notes, log_date)}
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc
...
async def get_wellness_report(user_id: str = DEFAULT_USER_ID, days: int = 30):
    try:
        return await asyncio.to_thread(wellness_report, user_id, days)
    except Exception as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc

@router.get("/wellness/notifications")
async def wellness_notifications(user_id: str = DEFAULT_USER_ID):
    try:
        return {"notifications": await asyncio.to_thread(wellness_due_prompts, user_id)}
    except Exception as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc

@router.get("/finance/watchlist")
async def finance_watchlist(user_id: str = DEFAULT_USER_ID):
    """Return live prices for the chat-configured stock watchlist."""
    try:
        return await asyncio.to_thread(finance_watchlist_report, user_id)
    except Exception as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc

@router.get("/finance/notifications")
async def finance_notifications(user_id: str = DEFAULT_USER_ID):
    """Return threshold alerts and the daily 09:15 market update."""
    try:
        return {"notifications": await asyncio.to_thread(finance_due_notifications, user_id)}
    except Exception as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc
...
async def start_task(task_id: str, request: TaskActionRequest):
    """Mark one task as actively being worked on."""
    try:
        result = await tasks_client.start_task(task_id=task_id, user_id=request.user_id)
        suggestion = await _record_action_events(
            conversation_id=request.conversation_id or f"ui:{request.user_id}",
            user_id=request.user_id,
            events=[build_tool_event(
                ...
                arguments={"task_id": task_id, "user_id": request.user_id, "status": "in_progress"},
                output=result,
                is_error=result.get("status") == "not_found",
            )],
        )
